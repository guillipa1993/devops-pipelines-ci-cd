...
def generate_prompt(log_type, language):
    ...
    if log_type == "failure":
        ...
        prompt = (
            ...
            "Ensure the analysis includes:\n"
            "1. **✅ Confirmation of Success:** Clearly state that the process completed successfully. Highlight the main factors contributing to the success. Use concise explanations.\n"
            "2. **🚀 Opportunities for Optimization:** Suggest specific areas for performance improvement or simplification. Include examples of improved code or configurations where applicable. Use the format:\n"
            "    ```python\n"
            "    # Example of optimization\n"
            "    optimized_code_here()\n"
            "    ```\n"
            "3. **📈 Scalability Recommendations:** Provide advice on extending the success to larger workloads or more complex use cases. Suggest code modularization, parameterization, or tool enhancements where appropriate. Include examples:\n"
            "    ```python\n"
            "    # Example of scalability improvement\n"
            "    scalable_function(param1, param2)\n"
            "    ```\n"
            "4. **🌿 Sustainability Suggestions:** Propose best practices to maintain success over time. Highlight tools or workflows, such as automated testing or monitoring. Provide concrete steps or commands where applicable.\n"
            "5. **🎉 Positive Feedback:** Acknowledge outstanding practices or results achieved. Celebrate the team's effort and motivate them to continue their great work.\n"
            "6. **✨ Best Practices:** Offer actionable tips to replicate this success. Use examples to illustrate best practices, such as:\n"
            "    ```python\n"
            "    # Example of a clean and maintainable function\n"
            "    def best_practice_example():\n"
            "        pass\n"
            "    ```\n"
            "Structure the output clearly and professionally with sections, bullet points, and emojis to make it engaging and actionable."
        )
    if language:
